...
var (
	...
	debExecutables = []debExecutable{
		...
		{
			BinaryName:  "goshic",
			Description: "Akashic CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Akashic private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Akashic account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "goshicstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		geth     = "goshic-" + basegeth + ext
		alltools = "goshic-alltools-" + basegeth + ext
	)
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "akashics/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "akashic/akashic")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "goshic-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "goshic-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "akashic"
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "goshic.exe" {
			gethTool = file
		} else {
	...
	build.Render("build/nsis.geth.nsi", filepath.Join(*workdir, "goshic.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("goshic-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "goshic.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "goshicstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	if *local {
		...
		os.Rename("goshic.aar", filepath.Join(GOBIN, "goshic.aar"))
		os.Rename("goshic-sources.jar", filepath.Join(GOBIN, "goshic-sources.jar"))
		return
	}
	...
	archive := "goshic-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("goshic.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "goshic-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Goshic.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Goshic.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "goshicstore/builds")`)
