	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				_, results = engine.VerifyHeaders(chain, []*types.Header{headers[i]}, []bool{true}, chain.db)
			} else {
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	for i := 0; i < len(preBlocks); i++ {
		_, results := runEngine.VerifyHeaders(chain, []*types.Header{preHeaders[i]}, []bool{true}, chain.db)
		// Wait for the verification result
		select {
		case result := <-results:
			if result != nil {
	...
	for i := 0; i < len(postBlocks); i++ {
		_, results := runEngine.VerifyHeaders(chain, []*types.Header{postHeaders[i]}, []bool{true}, chain.db)
		// Wait for the verification result
		select {
		case result := <-results:
			if result != nil {
	...
	_, results := runEngine.VerifyHeaders(chain, headers, seals, chain.db)
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals, chain.db)
			chain.Stop()
		} else {
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	abort, results := chain.engine.VerifyHeaders(chain, headers, seals, chain.db)
	close(abort)
