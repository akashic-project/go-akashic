...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		err := blockchain.engine.VerifyHeader(blockchain, block.Header(), true, blockchain.db)
		if err == nil {
...
func testHeaderChainImport(chain []*types.Header, blockchain *BlockChain) error {
	for _, header := range chain {
		...
		if err := blockchain.engine.VerifyHeader(blockchain, header, false, blockchain.db); err != nil {
			return err
		}
...
func TestEIP155Transition(t *testing.T) {
	...
	var (
		...
		gspec      = &Genesis{
			Config: &params.ChainConfig{ChainID: big.NewInt(11111), EIP150Block: big.NewInt(0), EIP155Block: big.NewInt(2), HomesteadBlock: new(big.Int)},
	...
	config := &params.ChainConfig{ChainID: big.NewInt(11112), EIP150Block: big.NewInt(0), EIP155Block: big.NewInt(2), HomesteadBlock: new(big.Int)}
